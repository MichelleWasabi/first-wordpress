...
class EventController{
    ...
    public function getEvents(){
        $query = Event::all([ 'post_status' => 'publish' ]);
        ...
        if ( $query->have_posts() ) {
        	while ( $query->have_posts() ) {
        		...
        		//forEach($query->meta_key as $keys => $value?? need to do forEach that unserializes the rsvp and do a rsvpNeg)
    ...
    function registerRSVP( $request ) {
        ...
        if( $query->have_posts() ){
            ...
            while ( $query->have_posts() ) {
                ...
                if ( isset( $data["user"] ) && isset( $data["answer"] ) ) {
                    
                    $completion = true;
                    
                    if( $data["answer"] === "yes" ){
                        $completion = $this->updateRSVPMetaFields("_rsvpYes", "_rsvpNo", $data["user"], $query->post->ID);
                    }else if($data["answer"] === "no"){
                        $completion = $this->updateRSVPMetaFields("_rsvpNo", "_rsvpYes", $data["user"], $query->post->ID);
                    }else{
